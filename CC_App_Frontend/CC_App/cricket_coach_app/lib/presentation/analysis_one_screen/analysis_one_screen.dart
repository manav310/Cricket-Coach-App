...
class AnalysisOneScreen extends StatefulWidget {
  const AnalysisOneScreen({super.key});

  @override
  AnalysisOneScreenState createState() => AnalysisOneScreenState();
}

class AnalysisOneScreenState extends State<AnalysisOneScreen> {
  ...
  Future<void> analyzeVideo(String videoPath) async {
    var uri = Uri.parse('http://172.17.22.194:8000/api/analyze/playstyle/');
    // For Android Emulator - "http://10.0.2.2:8000/api/analyze/playstyle/"
    var request = http.MultipartRequest('POST', uri)
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              CustomImageView(
                imagePath: ImageConstant.imganalysis1,
                height: 40.v,
                width: 44.h,
              ...
              Expanded(
                child: Text(
                  "Batsman Technique & Play style Analysis",
          ...
          Text(
            "Batsman position in crease (off, leg or middle), shot-type (horizontal or vertical), stance (balance) and footwork (aggressive or defensive).",
