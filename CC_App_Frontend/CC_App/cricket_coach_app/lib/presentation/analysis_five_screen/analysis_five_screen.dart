...
class AnalysisFiveScreen extends StatefulWidget {
  const AnalysisFiveScreen({super.key});

  @override
  AnalysisFiveScreenState createState() => AnalysisFiveScreenState();
}

class AnalysisFiveScreenState extends State<AnalysisFiveScreen> {
  ...
  Future<void> analyzeVideo(String videoPath) async {
    var uri = Uri.parse('http://172.17.22.194:8000/api/analyze/ball-speed/');
    // For Android Emulator - "http://10.0.2.2:8000/api/analyze/ball-speed/"
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final tempFile = File('${tempDir.path}/ball_speed_analysis.pdf');
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              CustomImageView(
                imagePath: ImageConstant.imganalysis5,
                height: 40.v,
                width: 44.h,
          ...
          Text(
            "It will calculate the speed of ball and will give a graph for user to analyze ball speed over time.",
  ...
  // Picking the Video File From Media
