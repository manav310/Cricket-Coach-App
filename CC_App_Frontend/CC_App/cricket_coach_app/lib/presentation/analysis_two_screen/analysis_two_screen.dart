...
class AnalysisTwoScreen extends StatefulWidget {
  const AnalysisTwoScreen({super.key});

  @override
  AnalysisTwoScreenState createState() => AnalysisTwoScreenState();
}

class AnalysisTwoScreenState extends State<AnalysisTwoScreen> {
  ...
  Future<void> analyzeVideo(String videoPath) async {
    var uri = Uri.parse('http://172.17.22.194:8000/api/analyze/timing/');
    // For Android Emulator - "http://10.0.2.2:8000/api/analyze/timing/"
  ...
  Future<void> downloadVideo(String videoUrl) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final file = File('${appDir!.path}/processed_video_${DateTime.now().millisecondsSinceEpoch}.mp4');
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              CustomImageView(
                imagePath: ImageConstant.imganalysis2,
                height: 40.v,
                width: 44.h,
          ...
          Text(
            "It will detect and follow the bat and ball to see, where the ball hits the bat and depending upon the position will give analysis.",
